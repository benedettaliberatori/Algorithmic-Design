...
class binheap(Generic[T]):
    ...
    def _swap_keys(self, node_a: int, node_b: int)-> None:
        ...
        tmp_ind = self.indexes[self._A[node_a][0]-1]
        self.indexes[self._A[node_a][0]-1]=self.indexes[self._A[node_b][0]-1]
        self.indexes[self._A[node_b][0]-1]=tmp_ind
    ...
    def decrease_key(self,old_value:T,new_value:T)->None: # modified to work without index 
        
        node= self.indexes[old_value-1]
